...
func NewQueue(sconf *structs.BatchQueue, broker queue.Broker, logger hclog.Logger) (queue.Queue, error) {
	switch sconf.Type {
	case structs.BatchQueueTypeDynamic:
		...
		return dynamic.NewDynamicPriorityQueue(broker, sconf, qconf, logger), nil
	case structs.BatchQueueTypeFifo:
		return fifo.NewFifoQueue(broker, logger), nil
	default:
