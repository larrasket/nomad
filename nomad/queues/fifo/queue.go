...
func NewFifoQueue(broker queue.Broker, logger hclog.Logger) *FifoQueue {
	return &FifoQueue{
...
func (f *FifoQueue) Start(ctx context.Context) error {
	go f.runProducer(ctx)
	go f.runConsumer(ctx)
	return nil
}
...
func (f *FifoQueue) SetEnabled(enabled bool, ss *state.StateStore) {
	if !enabled {
